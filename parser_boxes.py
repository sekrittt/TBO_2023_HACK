...
with open(os.path.join(current_dir, "dataset", "video2", "video2.txt"), 'r', encoding="utf-8") as f:
    data_list = [[float(j.strip()) if '.' in j else int(j.strip())
                  for j in i.strip().split(",")] for i in f]
